...
for file in smt2Files:
    ...
    for cmd, solver, error_pattern in [ #TODO: move error pattern to here
        ...
        try:
            ...
            result = p.stdout.decode("utf-8", errors="ignore").splitlines()[0] if p.stdout else ""
        ...
        if p.stderr is not None and len(p.stderr) > 0:
            ...
            print(p.stderr.decode("utf-8", errors="ignore"), end='')
            print("\033[92m", end='') # green color
            print(p.stdout.decode("utf-8", errors="ignore"), end='')
        ...
        # error pattern for each solver
            ...
            invalidCount += 1
            print(f"\nInvalid file for {solver}: {file}")
            err = p.stdout.splitlines()[0].decode("utf-8", errors="ignore")
        else:
            validCount += 1
        ...
        json_array_item[solver + "_stdout"] = p.stdout.decode("utf-8", errors="ignore")
        json_array_item[solver + "_stderr"] = err
        

    with open(file, 'r') as f:
        json_array_item["undefined_function_count"] = f.read().count("declare-fun")
        json_array_item["assertion_count"] = sum(1 for line in f.readlines() if line.startswith('(assert'))
